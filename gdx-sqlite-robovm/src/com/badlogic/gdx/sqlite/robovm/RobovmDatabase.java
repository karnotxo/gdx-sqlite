...
/**
 * RoboVM database implementation (simplified, minimal error handling retained).
 * Author: truongps
 */
public class RobovmDatabase implements Database {
    static final String TAG = "RobovmDatabase";

    /** Location of database (legacy test path). */
    static final String DB_URL =
            "sqlite:/" + (new File(System.getenv("HOME"), "Library/test.db")).getAbsolutePath();
    ...
    public void openOrCreateDatabase() throws SQLiteGdxException {
        ...
        try {
            String DB_URL =
                    "sqlite:/" + (new File(System.getenv("HOME"), "Library/" + dbName)).getAbsolutePath();
    ...
    @Override
    public com.badlogic.gdx.sqlite.PreparedStatement getPreparedStatement(String query)
            throws SQLiteGdxException {
        throw new UnsupportedOperationException("Prepared statements not implemented for RoboVM backend");
    }

    @Override
    public long getLastRowId() throws SQLiteGdxException {
        throw new UnsupportedOperationException("Retrieving last row id not implemented for RoboVM backend");
    }
}
